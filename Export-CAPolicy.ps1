...
# ExportLocation
$ExportLocation = "C:\Users\BrianReid\CAPolicy.html"
...
if ($null -eq $MgContext) {
    Write-host "Connect-MgGraph" -ForegroundColor Cyan
...
$AdNames = @{}
$batchSize = 1000
$totalIds = $ADsearch.Count
for ($i = 0; $i -lt $totalIds; $i += $batchSize) {
    $currentBatch = $ADsearch[$i..([math]::Min($i + $batchSize - 1, $totalIds - 1))]
    Get-MgDirectoryObjectById -ids $currentBatch | ForEach-Object {
        $obj = $_.Id
        #$disp = $_.displayName
...
# Sort CAExport by policy Name (alphabetically)
$CAExport = $CAExport | Sort-Object Name
...
$sort = "Name", "PolicyID", "Status", "Users", "UsersInclude", "UsersExclude", "TargetResources", "ApplicationsIncluded", "ApplicationsExcluded", `
    ...
    "PlatformsExclude", "ClientApps", "Devices", "DevicesIncluded", "DevicesExcluded", "DeviceFilters", "GrantControls", "BuiltInControls", "TermsOfUse", `
    "CustomControls", "GrantOperator", "SessionControls", "SessionControlsAdditionalProperties", "ApplicationEnforcedRestrictionsIsEnabled", `
    "ApplicationEnforcedRestrictionsAdditionalProperties", "CloudAppSecurityType", "CloudAppSecurityIsEnabled", "CloudAppSecurityAdditionalProperties", `
    "DisableResilienceDefaults", "PersistentBrowserIsEnabled", "PersistentBrowserMode", "PersistentBrowserAdditionalProperties", `
    "SignInFrequencyAuthenticationType", "SignInFrequencyInterval", "SignInFrequencyIsEnabled", "SignInFrequencyType", "SignInFrequencyValue", `
