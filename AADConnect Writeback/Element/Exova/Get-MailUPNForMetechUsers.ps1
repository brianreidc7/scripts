...
$ExovaMEDC = "USNJDCS095.exova-me.local"
    ...
    $SourceUsers = Import-CSV iCIMS-Metech.csv
    ...
    foreach ($SourceUser in $SourceUsers) {
        ...
        $TargetUser = Get-ADUser -Properties mail -Filter {mail -eq $MetechUser } -Server $MetechDC
    ...
    # Export to CSV 
    $resultsarray| Export-csv iCIMS-Metech-Updated.csv -notypeinformation
    ...
    ## Get DN of user in Exova-EU and build spreadsheet
    $SourceUsers = Import-CSV CSOD-Metech.csv
    ...
    foreach ($SourceUser in $SourceUsers) {
        ...
        $TargetUser = Get-ADUser -Properties mail -Filter {mail -eq $ExovaUser } -Server $MetechDC
    ...
    # Export to CSV 
    $resultsarray| Export-csv CSOD-Metech-Updated.csv -notypeinformation
    ...
    $SourceUsers = Import-CSV iCIMS-Metech-Updated.csv
 
    foreach ($SourceUser in $SourceUsers) {

       # Write-Host $SourceUser.ExovaEUDN
        Get-ADUser $SourceUser.MetechDN -Server $MetechDC -Property Mail | Select Name,Mail,UserPrincipalName # Cannot append to CSV as version of PS is too old on these machines
        #Set-ADUser $SourceUser.MetechDN -EmailAddress $SourceUser.NewEmailAndUsername -Server $MetechDC 
    ...
    $SourceUsers = Import-CSV CSOD-Metech-Updated.csv
